...
def monte_carlo_evaluation(policy, env, every_visit=False, incremental_mean=True, stationary_env=True,
    ...
    3. If not using incremental mean store total return for each state over all episodes
       (again depending on First Visit MC vs Every Visit MC) and using this, update value function after all episodes have been run
    ...
    for episode in range(n_episodes):
        ...
        for idx, (state, reward) in enumerate(zip(episode_state_hist, episode_reward_hist)):
            if state not in visits_from_last_episode: # todo put all below, DRY
            ...
            # todo check that return is just summed up together and then only at end of episode do you do error term update (not for each update)
...
if __name__ == '__main__':
    ...
    value0 = monte_carlo_evaluation(policy0, env, True)
    print(value0)
